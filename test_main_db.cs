...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            var context = scope.ServiceProvider.GetRequiredService<OGRALAB.Data.OgralabDbContext>();
            var authService = scope.ServiceProvider.GetRequiredService<OGRALAB.Services.IAuthenticationService>();
    ...
    private static IHostBuilder CreateHostBuilder(IConfiguration configuration) =>
        Host.CreateDefaultBuilder()
            .ConfigureServices((context, services) =>
            {
                ...
                services.AddDbContext<OGRALAB.Data.OgralabDbContext>(options =>
                    options.UseSqlite(configuration.GetConnectionString("DefaultConnection")), ServiceLifetime.Scoped);

                services.AddScoped<OGRALAB.Services.IAuthenticationService, OGRALAB.Services.AuthenticationService>();
            });
